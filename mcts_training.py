...
args = dotdict({
    ...
    'show_screen': True, 
    'n_in_rows': 4, 
    'exploration_rate': 0.2,    # exploration rate for self-play
    'exp_rate': 0.0,            # experimental rate
    '_is_selfplay': True,       # if true, then self-play, else, then test
    'numIters': 1000,           # number of iterations
    'nCompare': 100,             # Number of games to play during arena play to determine if new net will be accepted.
    ...
    'cpuct': 1,                 # a heuristic value used to balance exploration and exploitation.
    'checkpoint': './temp/',    # Directory to save the checkpoints.
    'load_model': False,         # Whether to load the pre-trained model.
    'load_folder_file': ('Models','nnet5.pt'), # (folder,file) to load the pre-trained model from.
